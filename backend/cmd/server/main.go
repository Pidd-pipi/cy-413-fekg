...
func main() {
	...
	if e = db.AutoMigrate(&model.User{}, &model.Mood{}, &model.Assessment{}, &model.Journal{}, &model.UserAssessment{}, &model.WeeklyReport{}); e != nil {
	...
	wr := repository.NewWeeklyReportRepository(db)
	...
	ws := service.NewWeeklyReportService(wr, mr, jr, logger)
	if e = as.Seed(); e != nil {
		logger.Error("assessment seed failed", "error", e)
		os.Exit(1)
	}
	h := router.Handlers{User: handler.NewUserHandler(us, as, logger, cfg.JWTSecret, cfg.JWTIssuer), Mood: handler.NewMoodHandler(ms, logger), Assessment: handler.NewAssessmentHandler(as, logger), Journal: handler.NewJournalHandler(js, logger), WeeklyReport: handler.NewWeeklyReportHandler(ws, logger)}
